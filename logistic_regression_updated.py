...
def main(train='train.csv', test='test.csv', submit='logistic_pred.csv'):    
	...
	#good_features = [19,60]
	good_features = [0,8,9,10,36,37,38,41,41,43,47,53,60,61,63,64,67,69,71,75,85]
	#good_features = [1]#, 1, 7, 8, 9, 10, 36, 37, 38, 41, 42, 43, 47, 51, 53, 56, 60, 61, 63, 64, 66, 67, 69, 71, 75, 79, 85, 91]
	...
	for i in range(X_train_all.shape[0]):
		...
		for j in good_features:
			f.write(' ' + str(c) + ':' + str(X_train_all[i][j]))
			c += 1
		f.write('\n')
	f.close()
	print 'printed features to file'
	...
	avg_auc = cv_loop(X_train, y, model, 10)
	print 'Average AUC score:',avg_auc
	'''print cross_val_score(model, X_train, y, scoring="roc_auc", n_jobs=2, verbose=1)
	...
	for train_index, test_index in skf:
		...
		for i in train_index:
			X_train_k.append(X_train[i].toarray())
			Y_train_k.append(y[i])
		for i in test_index:
			#X_test.append(list(X.ix[i]))
			X_test_k.append(X_train[i].toarray())
			Y_test_k.append(y[i])
		print '========== Fold %d ==========' % (fold)
		model.fit(X_train_k, Y_train_k)
		pred = model.predict_proba(X_test_k)[:,1]
		result = util.calculate_auc(Y_test_k, pred)
		result_sum += result
